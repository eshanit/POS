...
const props = defineProps<{ product: any, categories: any[], suppliers: any[] }>()
...
const form = useForm({
  ...
  supplier_id: props.product.supplier_id?.toString() || '',
  cost_price: props.product.cost_price?.toString() || '',
  price_usd: props.product.price_usd?.toString() || '',
  price_zig: props.product.price_zig?.toString() || '',
  reorder_level: props.product.reorder_level?.toString() || '',
  expiry_date: props.product.expiry_date || '',
})

const submit = () => {
  form.put(productsRoutes.update.url(props.product.id))
}
</script>
